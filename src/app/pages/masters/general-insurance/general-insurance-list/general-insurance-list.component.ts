...
@Component({
  selector: 'app-general-insurance-list',
...
export class GeneralInsuranceListComponent implements OnInit {
  ...
  constructor(
    ...
    this.router.events
      ...
      .subscribe((event: any) => {
        this.isChildRouteActive = event.url.indexOf('masters/general-insurance/new') !== -1 || event.url.indexOf('masters/general-insurance/edit') !== -1;

        if (!this.isChildRouteActive) {
          this.breadcrumbTitle = 'List';
          this.pageTitle = 'General Insurance List';
        }
        else {
          if (event.url.indexOf('masters/general-insurance/new') !== -1) {
            this.breadcrumbTitle = 'New';
            this.pageTitle = 'Create New General Insurance';
          }
          else if (event.url.indexOf('masters/general-insurance/edit') !== -1) {
            this.breadcrumbTitle = 'Edit';
            this.pageTitle = 'General Insurance Info';
          }
          else {
          }
        }
    ...
    this.insertSubscription = recordCreationService.recordInserted$.subscribe(
      record => {
        if (record.table == 'General Insurance') {
          var newRowData = {
            id: record.id,
            name: record.name,
          }
  ...
  onGridReady(params: any) {
    ...
    this.columnDefs = [
      ...
      { field: 'name', headerName: 'Name', sortable: true, filter: true, resizable: true, width: 180 },
    ];
  }
  ...
  onEdit(e: any) {
    this.router.navigate(['masters/general-insurance/edit/' + e.rowData.id]);
  }
  ...
  onCreate() {
    this.router.navigate(['masters/general-insurance/new']);
  }
